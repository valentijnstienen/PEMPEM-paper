...
def create_extended_graph(GLOBAL_SETS):
    SEED, shuf, CASE, CASENAME, preProcessTrajectories, PATH_TO_PRE_PROCESSED_TRAJECTORIES, (initializeGraph, area), (leaveout_part, create_compare_graph_not_excluding, \
    ...
    dat_range, load, save, save_thresh, rest_time, viewData, do_print, (plot, editing), mapbox_accesstoken = GLOBAL_SETS
    ...
    set_string = set_string + str(settings[2]) + "_" + str(LO_CASE)
    ...
    # Remove part of the OSM graph
    #mid_point, radius = Point((102.53609, -0.76089)), 1500 #meters #Point((225950, -84350))
    #mid_point = ox.projection.project_geometry(mid_point, to_crs=used_crs, to_latlong=False)[0]
    # Example 1: Point((197576.465203074127203, -74504.796587277232902))
    # Example 2: Point((221576.465203074127203, -80504.796587277232902))
    # Example 3: Point((221576.465203074127203, -56504.796587277225626))
    # Example 4: Point((185576.465203074127203, -50504.796587277225626))
    if leaveout_part:
        mid_point_LO = Point(mid_point_LO)
        # mid_point_LO = ox.projection.project_geometry(mid_point_LO, to_crs=used_crs, to_latlong=False)[0]

        EXCLUDED_POLYGON = Polygon([Point(x) for x in [(mid_point_LO.x-radius_LO, mid_point_LO.y+radius_LO), (mid_point_LO.x+radius_LO, mid_point_LO.y+radius_LO), (mid_point_LO.x+radius_LO, mid_point_LO.y-radius_LO), (mid_point_LO.x-radius_LO, mid_point_LO.y-radius_LO),(mid_point_LO.x-radius_LO, mid_point_LO.y+radius_LO)]])
        # find the nodes/edges in this polygon
        nodes, edges = ox.graph_to_gdfs(G, nodes = True, node_geometry = True)

        # Only remove the OSM part when necessary (not when computing the compare graph)
        if not create_compare_graph_not_excluding: 
            nodes_to_remove = nodes[nodes.within(EXCLUDED_POLYGON)].index
            G.remove_nodes_from(nodes_to_remove)
            nodes, edges = ox.graph_to_gdfs(G, nodes = True, node_geometry = True)
            edges_to_remove = edges.loc[edges.intersects(EXCLUDED_POLYGON), ['u','v','key']].values.tolist()
            G.remove_edges_from(edges_to_remove)
            
        # Create and save as shapefiles
        if save_shapefiles & (not create_compare_graph_not_excluding):
            nodes, edges = ox.graph_to_gdfs(G)
            if not os.path.exists("Results/"+CASE+"/"+CASENAME+"/"+ set_string+"/_Shapefiles (Start)"): os.makedirs("Results/"+CASE+"/"+CASENAME+"/"+ set_string+"/_Shapefiles (Start)")
            create_shapefile(nodes, "Results/"+CASE+"/"+CASENAME+"/"+ set_string+"/_Shapefiles (Start)/start_nodes_without.shp")
            create_shapefile(edges, "Results/"+CASE+"/"+CASENAME+"/"+ set_string+"/_Shapefiles (Start)/start_edges_without.shp")
    ...
    errors, skips, trips, relevant_IDs = [], [], [], []

    # Add artificial data when Speed, Driven, and Datetime are not included
    if settings[5]:
        points['Speed'] = 0
        points['DateTime'] = 0
        nx.set_edge_attributes(G, False, name = 'driven')
        nx.set_edge_attributes(G, "", name = 'DatesVelocities')


    for new_id in new_ids:
        if (new_id) % 100 == 0: print("Adding ID " + str(new_id) + " to the road network.")
        ...
        # If GPS trace will not affect the excluded region, continue with the next GPS trace
        if leaveout_part:
            if not EXCLUDED_POLYGON.intersects(used_polygon): 
                existing_ids = existing_ids + [new_id]
                if save & ((new_id+1) % save_thresh == 0):
                    if not os.path.exists("Results/"+CASE+"/"+CASENAME+"/"+set_string): os.makedirs("Results/"+CASE+"/"+CASENAME+"/"+set_string)
                    with open("Results/"+CASE+"/"+CASENAME+"/" + set_string + "/graph_"   + str(existing_ids[0]) + "-"+ str(str(existing_ids[len(existing_ids)-1])) + ".pickle", "wb") as file: pickle.dump(G, file)
                ...
                if not create_compare_graph_not_excluding: 
                    if not os.path.exists("Results/"+CASE+"/"+CASENAME+"/"+set_string): os.makedirs("Results/"+CASE+"/"+CASENAME+"/"+set_string)
                    with open("Results/"+CASE+"/"+CASENAME+"/" + set_string + "/relevant_IDs_"+str(LO_CASE)+".txt", "w") as output: output.write(str(relevant_IDs))
    ...
    if save & (len(new_ids) > 0):
        if not os.path.exists("Results/"+CASE+"/"+CASENAME+"/"+set_string): os.makedirs("Results/"+CASE+"/"+CASENAME+"/"+set_string)
        if leaveout_part:
            if create_compare_graph_not_excluding: 
                with open("Results/"+CASE+"/"+CASENAME+"/"+set_string +"/graph_" +str(LO_CASE)+"_compare.pickle", "wb") as file: pickle.dump(G, file)
            else: 
                with open("Results/"+CASE+"/"+CASENAME+"/"+set_string +"/graph_" + str(existing_ids[0]) + "-"+ str(str(existing_ids[len(existing_ids)-1])) + "_"+str(LO_CASE)+".pickle", "wb") as file: pickle.dump(G, file)
            if not save_shapefiles: return
    ...
    if save: 
        if leaveout_part:
            if create_compare_graph_not_excluding: 
                if not os.path.exists("Results/"+CASE+"/"+CASENAME+"/"+set_string+"/_Graphs_C"): os.makedirs("Results/"+CASE+"/"+CASENAME+"/"+set_string+"/_Graphs_C")
                nodes.to_csv("Results/"+CASE+"/"+CASENAME+"/"+set_string+"/_Graphs_C/Nodes_"+ str(existing_ids[0]) + "_"+ str(str(existing_ids[len(existing_ids)-1]))+".csv", sep = ";")
                edges.to_csv("Results/"+CASE+"/"+CASENAME+"/"+set_string+"/_Graphs_C/Edges_"+ str(existing_ids[0]) + "_"+ str(str(existing_ids[len(existing_ids)-1]))+".csv", sep = ";")
            else:
    ...
    if save: 
        if leaveout_part:
            if create_compare_graph_not_excluding: 
                if not os.path.exists("Results/"+CASE+"/"+CASENAME+"/"+set_string+"/_Shapefiles_C"): os.makedirs("Results/"+CASE+"/"+CASENAME+"/"+set_string+"/_Shapefiles_C")
                create_shapefile(nodes, "Results/"+CASE+"/"+CASENAME+"/"+set_string+"/_Shapefiles_C/SF_nodes.shp")
                create_shapefile(edges, "Results/"+CASE+"/"+CASENAME+"/"+set_string+"/_Shapefiles_C/SF_edges.shp")
                return
    ...
    if plot: 
        ...
        # Draw basemap (OSM)
        fig.update_layout(mapbox1 = dict(center = dict(lat=points_new.Latitude.mean(), lon=points_new.Longitude.mean()), accesstoken = mapbox_accesstoken, zoom = 13), margin = dict(t=10, b=0, l=10, r=10),showlegend=False,mapbox_style="satellite") # or use satellite as mapbox_style
        ...
        if editing & (len(new_ids) > 0):
            ...
            if leaveout_part:
                EXCLUDED_POLYGON = ox.projection.project_geometry(EXCLUDED_POLYGON, crs = used_crs,  to_latlong = True)[0]
        ...
        if leaveout_part:
            x, y = EXCLUDED_POLYGON.exterior.coords.xy
            fig.add_trace(go.Scattermapbox(mode='lines', lat=y.tolist(), lon=x.tolist(), visible = True,  marker = {'size' : 20, 'color': 'green', 'allowoverlap': True}))
        try:
...
# if leaveout_part: create_extended_graph(point_excluded = mid_point_LO, radius = radius_LO, casename_LO = LO_CASE)
# else: create_extended_graph(point_excluded = None, radius = None, casename_LO = None)
