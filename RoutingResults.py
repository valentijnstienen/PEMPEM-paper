...
# Location   : cd "Desktop/NETX/Routing results/"
# Run        : python RoutingResults.py
import pickle
import geopandas
import pandas as pd
import osmnx as ox
...
# Define CASE
CASE = '00' # 00, 0,1,2     # SEED 
MAX_PROJECTION = 30 # 30, 50, 70, 90
# If we want to find unique OD pairs
FIND_UNIQUE_ODS = True
CLOSENESSRADIUS = 20

# Load graphs
with open(r"graph_0-0.pickle", "rb") as input_file: G_original = pickle.load(input_file)
with open(r"SP_Results/graph_0-14960_" + CASE + ".pickle", "rb") as input_file: G_extended = pickle.load(input_file)
...
fromto_points = pd.read_csv("PEMPEM_stops_125_3_80_10_5_5_ARTCOURSE_DIRECTION_NOTALWAYS_1311.csv", sep = ";", index_col = 0, low_memory = False)
...
if FIND_UNIQUE_ODS: 
    ...
    for from_point, to_point in zip(from_points, to_points):
        print("Working on point: " + str(ind))
        """----------------------------------------------------------------------------------"""
        add = True
        """----------------------------------------------------------------------------------"""
...
idn = 0
for from_point, to_point in zip(from_points, to_points):
    print("Working on point: " + str(idn))
    ...
    #plot_path(G_original_projected, from_point, to_point, SP_edges_OLD)
    """----------------------------------------------------------------------------------"""
    """----------------------------------------------------------------------------------"""
    ...
    #plot_path(G_extended_projected, from_point, to_point, SP_edges_NEW)
    """----------------------------------------------------------------------------------"""
    """----------------------------------------------------------------------------------"""   
    ...
    idn += 1
    if idn % 100 == 0:
       df.to_csv("SP_Results/SP_diffs_"+CASE+"_"+str(MAX_PROJECTION)+"_NEW.csv", sep = ";")
