...
def createSubtrips_PEMPEM(filename, file_ID):
    """
    ...
    df_full = pd.read_csv(filename, sep = ";", index_col = 0, low_memory=False) 
    ...
    subtrips.drop(['Track way', 'subID', 'tripID'], axis='columns', inplace=True)
    ...
    #subtrips.to_csv('PEMPEM_subtrips_416.csv', sep = ";")
    subtrips.to_csv('PEMPEM/PEMPEM_subtrips_' + str(MAX_DOWN_TIME) + '_' + str(DOWN_SPEED) + '_' + str(MAX_SPEED) +'_'+ str(MIN_ACF_DISTANCE_COVERED) + '_' + str(MIN_TIME_BETWEEN_POINTS) + '_' + str(MIN_SUBTRIP_LENGTH) + '_ARTCOURSE_DIRECTION_NOTALWAYS_1311.csv', sep = ";")
    df_fromto.to_csv('PEMPEM/PEMPEM_stops_' + str(MAX_DOWN_TIME) + '_' + str(DOWN_SPEED) + '_' + str(MAX_SPEED) +'_'+ str(MIN_ACF_DISTANCE_COVERED) + '_' + str(MIN_TIME_BETWEEN_POINTS) + '_' + str(MIN_SUBTRIP_LENGTH) + '_ARTCOURSE_DIRECTION_NOTALWAYS_1311.csv', sep = ";")
    
def createSubtrips_WORLDBANK(filename, fname, file_ID, subtrip_threshold):
    """
    This function creates the DataFrame that we will work with. 
    ...
    ----------
    filename : string
        file name, e.g. PEMPEM_data when loading full dataset, PEMPEM_data_29 is a subset (can be used to reduce waiting time).
    file_ID : list (consisting of numbers)
        list with the IDs to be incorporated. One ID corresponds to one vehicle on one day (one excel file)
    subtrip_threshold : number
        minimal number of points in a subtrip
    long_threshold : number
        minimal length of an edge before it is split into multiple smaller edges. 
    ...
    subtrips : pandas DataFrame 
        ...
        A sample of this dataframe
        DateTime                Latitude   Longitude  Speed Course Track way  ID subID tripID
        0                    0 -0.758459  102.539806      0     -1       GPS  29     1   29_1
        1  2019-07-30 10:46:44 -0.756030  102.537750  33.91    332       GPS  29     1   29_1
        2  2019-07-30 10:46:51 -0.755490  102.537490  34.98    333       GPS  29     1   29_1
        3  2019-07-30 10:47:51 -0.750790  102.534690  20.81    326       GPS  29     1   29_1
        4  2019-07-30 10:48:50 -0.749410  102.533910  34.65    326       GPS  29     1   29_1
    """
    ...
    subIDs_rem = list(counttab.index[counttab > subtrip_threshold])
    ...
    # Save the dataset
    subtrips.to_csv('WorldBank/' + fname + '.csv', sep = ";")
...
if CASE == "PEMPEM": # PEMPEM data
    createSubtrips_PEMPEM('PEMPEM/PEMPEM_data.csv', range(0,2656)) #2656
else: # WorldBank data
    if case == 'OSM': createSubtrips_WORLDBANK('WorldBank/coordinates_OSM_FULL_0609.csv', 'WorldBank_subtrips_0609_OSM', range(0,8001), 1) #8001         # Transform OSM into GPS traces
    else: createSubtrips_WORLDBANK('WorldBank/coordinates_estrada_FULL_0609.csv', 'WorldBank_subtrips_0609_estrada', range(0,1101), 1) #1101 # Transform ESTRADA into GPS traces